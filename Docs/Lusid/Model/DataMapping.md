# Finbourne.Sdk.Lusid.Model.DataMapping
...
|------|------|----------|-------------|
| **DataDefinitions** | [List&lt;DataDefinition&gt;](DataDefinition.md) | Optional | A map from LUSID item keys to data definitions that define the names, types and degree of uniqueness of data provided to LUSID in structured data stores. |
...
var instance = new DataMapping(
    dataDefinitions: new List<DataDefinition>()  // optional — A map from LUSID item keys to data definitions that define the names, types and degree of uniqueness of data provided to LUSID in structured data stores.
);
```
### Serializing to JSON
...
```csharp
var instance = JsonConvert.DeserializeObject<DataMapping>(json);
```


## Related Models

- [DataDefinition](DataDefinition.md) — used in `DataDefinitions`
