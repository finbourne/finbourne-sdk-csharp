...
| **CompletionRatio** | **decimal?** | Optional | Reviewed / (Reviewed + Required). Is 1.0 when the denominator is zero, and null when execution failed. |
...
var instance = new RecReview(
    ...
    completionRatio: 0.0d  // optional — Reviewed / (Reviewed + Required). Is 1.0 when the denominator is zero, and null when execution failed.
);
```
### Serializing to JSON
