...
var instance = new OrderFlowConfiguration(
    includeEntityTypes: "..."  // required — Controls whether Orders and Allocations orders are included in the Portfolio valuation.  Valid values are  None (to account for Transactions only), Allocations (to include Allocations and Transactions) and  OrdersAndAllocations (to include Orders, Allocations and Transactions). Available values: None, Allocations, OrdersAndAllocations.
);
```
### Serializing to JSON
