# Finbourne.Sdk.Scheduler.Model.CreateJobRequest
...
| **DateCreated** | **DateTimeOffset** | Optional | Date when job was created. Defaults to now. |
...
var instance = new CreateJobRequest(
    jobId: new ResourceId(...),  // required
    name: "...",  // required — Name of the job
    author: "...",  // optional — Author of the job
    dateCreated: DateTimeOffset.Now,  // optional — Date when job was created. Defaults to now.
...
```csharp
var instance = JsonConvert.DeserializeObject<CreateJobRequest>(json);
```
