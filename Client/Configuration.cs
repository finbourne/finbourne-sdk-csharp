...
namespace Finbourne.Sdk.Client
{
    ...
    public class Configuration : IReadableConfiguration
    {
        ...
        public const string Version = "0.1.113";
        ...
        public static readonly ExceptionFactory DefaultExceptionFactory = (methodName, response) =>
        {
            ...
            IList<string> requestId = null;
            IList<string> correlationId = null;

            if (response.Headers != null)
            {
                response.Headers.TryGetValue("lusid-meta-requestid", out requestId);
                response.Headers.TryGetValue("lusid-meta-correlationid", out correlationId);
            }

            if (!string.IsNullOrEmpty(response.RawContent))
            {
                // Returns null if it doesn't exist
                var lusidProblemDetails = ApiExceptionExtensions.DeserializeLusidProblemDetails(response.RawContent);

                errorMessage = FormatErrorMessage(methodName ,response.ErrorText, response.RawContent, requestId,
                    correlationId, lusidProblemDetails);
                return new ApiException(status, errorMessage, response.RawContent, response.Headers, lusidProblemDetails, requestId, correlationId);
            }

            errorMessage = FormatErrorMessage(methodName, response.ErrorText, requestId: requestId,
        ...
        private static string FormatErrorMessage(
            ...
            IList<string> requestId = null,
            IList<string> correlationId = null,
            LusidProblemDetails lusidProblemDetails = null
            )
        {
            var errorMessage = new StringBuilder();

            errorMessage.AppendLine($"{methodName} {errorText}");

            if (requestId != null && requestId.Count > 0)
                errorMessage.AppendLine($"RequestId: {string.Join(", ", requestId)}");

            if (correlationId != null && correlationId.Count > 0)
                errorMessage.AppendLine($"CorrelationId: {string.Join(", ", correlationId)}");
            if (lusidProblemDetails != null)
            {
                errorMessage.AppendLine($"Lusid logs: {lusidProblemDetails.Instance}");
                errorMessage.AppendLine($"Lusid Error Code: {lusidProblemDetails.Type}");
            }

            if(!string.IsNullOrEmpty(rawContent))
                errorMessage.AppendLine($"{rawContent}");
        ...
        {
            Proxy = null;
            UserAgent = WebUtility.UrlEncode("OpenAPI-Generator/0.1.113/csharp");
        ...
        {
            string report = "C# SDK (Lusid) Debug Report:\n";
            report += "    OS: " + System.Environment.OSVersion + "\n";
            report += "    .NET Framework Version: " + System.Environment.Version  + "\n";
            report += "    Version of the API: 1.0.0\n";
            report += "    SDK Package Version: 0.1.113\n";
